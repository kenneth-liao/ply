...
test("measure includes the shadow extent in painted bounds and clipping", async () => {
  ...
  expect(layer.effects).toEqual({ shadow: [{ dx: 10, dy: 10, blur: 0, color: "#000000" }], outline: null, innerShadow: null });
  ...
  expect(JSON.parse(measure3.stdout).layers[0].effects).toEqual({ shadow: null, outline: null, innerShadow: null });
