...
export interface RetainedLayerReview {
  layerId: string;
  reviewPath: string;
  /** The reviewed candidate's verified bytes — null for a shape Layer (#208),
   *  whose content is its parameters and which retains no bytes. */
  candidate: { label: string; contentHash: string; bytes: Buffer } | null;
  references: EvidenceReference[];
...
export async function reviewRetainedLayer(
  ...
  // The vector colour (#215): captured inside the lock from the resolved
  // revision — a fact row whenever the reviewed revision carries one. The
  // sheet shows the retained (authored) pixels, so the row names the
  ...
  const review = await withProjectLock(resolvedRoot, async () => {
    ...
    if (rev.kind === "image" && rev.vectorColor !== undefined) {
      vectorColorFact = rev.vectorColor;
    }
    ...
    if (rev.kind === "shape") {
      ...
      // those parameters as facts rows, with no candidate or matte figure:
      // there is no pixel evidence to display, and inventing one is exactly
      ...
      const sheet = renderEvidenceSheet({
        ...
        subtitle: "shape Layer — content is its parameters; there are no retained pixels to review",
        facts,
        references: [],
        candidates: [],
        mattes: [],
  ...
  // A shape Layer's review was built and published inside the lock (the
  // parameter facts sheet, no candidate); there is no evidence pipeline to
  // run for it — return the resolved review directly.
  ...
  if (vectorColorFact !== null) {
    facts.push(["vector colour", `${vectorColorFact} (paint-time over the retained bytes)`]);
  }
  if (gradeFact !== null) {
