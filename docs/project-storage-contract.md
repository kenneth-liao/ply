...
├── matting/                     # Retained Matting provenance (#108), created on first retention
│   └── <matte-id>/
│       ├── matte.json           # The published matte record's verbatim bytes
│       └── sources/             # Retained published source copy (#161: version 2 inference only, when distinct)
│           └── <sha256>.png
...
#### `matting/<matte-id>/matte.json` (Owned by #108, spec #102 US-003)

A matted image Layer's authoritative Matting provenance is retained inside the Project as the published matte record's **verbatim bytes**, read from `out/matting/<matteId>/matte.json` (see docs/matting-publication-contract.md). There is exactly one retained representation of those facts: nothing from the record (source identity, engine, alpha report, warnings, output) is embedded in Layer revisions, Composition documents, or Render manifests.

- **Verbatim retention**: an existing retained record must be byte-identical to the published record being ingested or the ingestion is refused — retained provenance is never rewritten. Later Layer replacements retain *other* matte records; earlier records and the content blobs they pin remain byte-identical.
- **Derived linkage, one identity**: a Layer revision links to its Matting provenance only through its `contentHash` — the revision's content identity, the retained blob in `content/<sha256>`, and the record's output `contentHash` are the same sha-256 fact. No matte-id pointer is stored on the revision; the linkage is derived by matching a revision's `contentHash` against each retained record's output. Zero matches means the bytes are not matted content (an ordinary image Layer); more than one match (two retained mattes claiming identical bytes) fails closed rather than reporting possibly wrong provenance; an unreadable retained record also fails closed.
- **Derived generation lineage (#107 reuse, no second copy of request facts)**: a matte whose source was itself a published Generation Job output is linked by the same sha-256 fact — the matte record's `request.source.contentHash` matches the job record's output `contentHash`. When exactly one published job matches, that job's record is retained verbatim under `generation/<job-id>/job.json` through the existing #107 machinery, so both Matting lineage and predecessor generation provenance resolve offline; the request facts keep their one home (the generation record). Zero matches means the matte's source was an ordinary local image: no generation record is retained. More than one match is ambiguous lineage and the ingestion fails closed before anything is retained.
- **No second lifecycle**: a matted-content Layer is an ordinary image Layer. No matted-Layer identity, category, or approval fields exist; ingestion goes through the exact canonical Layer publication protocol below.
- **Created on first retention**: Projects created before #108 have no `matting/` directory and remain valid; it appears with the first matted-content ingestion. The retained set is the verbatim record plus, for version 2 inference records, the published source copy (`sources/<sha256>.png`, #161) — the matted pixels live in `content/<sha256>` like every other Layer's bytes, so the Layer renders, edits, forks, and replays offline after the external Matting files and the original source are removed and the Project is relocated.
- **Retained source bytes (#161, spec #159 US-003)**: when the retained record is a version 2 inference record — the only record kind that names `request.source.file` — the exact published source bytes are retained beside the verbatim record at `matting/<matteId>/sources/<sha256>.png`, the same relative path the publication contract names. The copy is verified against the record's `request.source.contentHash` before staging, so it is that identity, not a second hash; an existing retained copy must be byte-identical or the ingestion is refused. After the caller's original file is deleted and the Project is relocated, rematting is ordinary `ply matte` on that retained path (no new command): it publishes a new matte id and leaves the old record unchanged. Native-alpha records store one blob (the output already is the source's bytes) and stage no `sources/` copy; version 1 records name no copy and gain none (no backfill) — a version 1 record naming a copy is refused as contradictory, fail-closed with nothing staged.
- **No garbage collection**: retained records and pinned blobs are never deleted by Ply (DEC-007, OOS-004).
...
- Retained provenance records (#107/#108) resolve through one canonical reader each (`resolveRetainedProvenance`, `resolveRetainedMattingProvenance`): records are parsed through the published-record parsers, matched by content identity, and never partially trusted — an ambiguous match (two retained records claiming one content identity) or an unreadable record fails closed instead of reporting possibly wrong provenance. The full derived chain (matted Layer → matte → predecessor generation job) resolves through `resolveRetainedMatteGenerationLineage`.
...
2. **Format & Decoding Verification**:
   ...
   - SVG inertness gate (#214, US-006): before anything is published, an SVG referencing anything outside itself is refused naming each reference and the fix (embed the resource as a data URI). The scan is generic — every attribute value on every element is judged (any `url(…)` occurrence anywhere, `href`/`src`/`data`/`base`/`poster`, each `srcset` candidate separately), plus `<style>` bodies, `<?xml-stylesheet?>`, and the DOCTYPE; values are normalized in parser order (XML entities decoded once, CDATA joined, CSS escapes unescaped) so encodings cannot hide a reference. DOCTYPE entity declarations (internal or external) and unterminated constructs (comment, CDATA, script, style, PI, tag) are refused as malformed-by-design; nested `data:image/svg+xml` payloads are re-scanned once. Data URIs and same-document fragments are accepted; a script never blocks import and never runs (`scanSvgExternalReferences` in `src/svg-inertness.ts`).
...
2. **Ingest and validate content**: for `--image`, decode the input image and compute SHA-256; for `--from-generation` (#107), resolve the published Generation Job record, select one output explicitly (a multi-output record without a selection is refused), read its file, verify its sha-256 against the recorded content identity, and decode the image; for `--from-matte` (#108), resolve the published matte record through the one published-record parser, read its single output file, verify its sha-256 against the recorded content identity, and — when the record names a distinct source copy — verify that copy's shape, containment, size, and sha-256 against the recorded source identity (a version 1 record naming a copy is refused as contradictory, fail-closed); decode the image, and resolve the predecessor generation linkage (an ambiguous match fails closed); for `--text`, resolve the bundled font family once and read its bundled bytes (unknown families and missing bundled bytes fail loudly, naming the bundled families). Everything that can refuse the source runs before any Project write.
3. **Stage immutable content blob** in `content/<sha256>` (atomic create if not already present) — the image bytes, the selected generated output's verified bytes, the verified matte output bytes, or the retained font bytes. For `--from-generation`, also stage the record verbatim under `generation/<jobId>/job.json` (atomic create; a present retained record must be byte-identical or the ingestion is refused) before any revision staging. For `--from-matte`, likewise stage the matte record verbatim under `matting/<matteId>/matte.json` plus, when the published record names a distinct source copy (version 2 inference), that verified copy under `matting/<matteId>/sources/<sha256>.png` (#161), plus the predecessor job record under `generation/<jobId>/job.json` when the matte's source was a published generation output. Provenance, pixels, and the revision publish under the same lock and rollback discipline.
...
- Pinned identifiers are strictly validated **before** any filesystem path is constructed from them (`rev_[0-9a-f]{16}` revision ids and `layer_[a-zA-Z0-9_]+` identities).
...
- `ply composition add <comp> <local-name> (--image <path> | --text <str> --font <family> [--font-size <px>] [--color <hex>] | --from-generation <job-id> [--output <n|sha256|prefix>] | --from-matte <matte-id>) [--x <x>] [--y <y>] [--opacity <op>] [options] [--json]`
  - `--from-generation` ingests one selected output of a published Generation Job (under `out/generation`, per docs/generation-publication-contract.md) as an ordinary image Layer and retains the record verbatim under `generation/<job-id>/job.json` (#107). Mutually exclusive with `--image`, `--text`, and `--from-matte`; a multi-output job without `--output` is refused. The output bytes are verified against the recorded sha-256 before ingestion; missing, corrupt, or mismatched sources fail without a live Layer reference.
  - `--from-matte` ingests the verified output of a published matte (under `out/matting`, per docs/matting-publication-contract.md) as an ordinary image Layer and retains the matte record verbatim under `matting/<matte-id>/matte.json` (#108). When the published record names a distinct source copy (version 2 inference), those verified bytes are retained beside the record at `matting/<matte-id>/sources/<sha256>.png` (#161) so a relocated Project can rematte without the original path; native-alpha ingests no second copy and version 1 records gain none. When the matte's source was a published Generation Job output (derived linkage by content identity), that job's record is retained verbatim under `generation/<job-id>/job.json` too; an ambiguous predecessor match fails closed before anything is retained. Mutually exclusive with `--image`, `--text`, and `--from-generation`. Missing, corrupt, or mismatched matte lineage fails without a live Layer reference; no engine runs and nothing generates.
...
- `ply layer review <layer-id> --out <path> [options] [--json]`
  - Builds the offline evidence review sheet for a generated or matted Layer
    ...
    only.
- `ply layer edit <layer-id> [--in-place | --fork --composition <comp> --use <local-name>] [--image <path> | --from-generation <job-id> [--output <n|sha256|prefix>] | --from-matte <matte-id> | --text <str> [--font <family>] [--font-size <px>] [--color <hex>]] [--x <x>] [--y <y>] [--opacity <op>] [options] [--json]`
