...
2. **Format & Decoding Verification**:
   ...
   - SVG inertness gate (#214, US-006): before anything is published, an SVG referencing anything outside itself — a remote or local image (`href`/`xlink:href`, both spellings, entity-decoded, including foreignObject content), a font or stylesheet (`@font-face`/`@import`/`url()`, `<?xml-stylesheet?>`, `<link>`), or an out-of-file `use` target — is refused naming each reference and the fix (embed the resource as a data URI). Data URIs and same-document fragments are accepted; a script never blocks import and never runs (`scanSvgExternalReferences` in `src/svg-inertness.ts`).
