...
- `ply composition add <comp> <local-name> (--image <path> | --text <str> --font <family> [--font-size <px>] [--color <hex>] | --from-generation <job-id> [--output <n|sha256>] | --from-matte <matte-id>) [--x <x>] [--y <y>] [--opacity <op>] [options] [--json]`
  ...
  - `--from-matte` ingests the verified output of a published matte (under `out/matting`, per docs/matting-publication-contract.md) as an ordinary image Layer and retains the matte record verbatim under `matting/<matte-id>/matte.json` (#108). When the matte's source was a published Generation Job output (derived linkage by content identity), that job's record is retained verbatim under `generation/<job-id>/job.json` too; an ambiguous predecessor match fails closed before anything is retained. Mutually exclusive with `--image`, `--text`, and `--from-generation`. Missing, corrupt, or mismatched matte lineage fails without a live Layer reference; no engine runs and nothing generates.
