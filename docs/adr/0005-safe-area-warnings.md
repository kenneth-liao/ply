...
- Status: Partially superseded by [ADR-0015](0015-caller-invoked-local-matting-and-regions.md): caller-supplied region scope replaces hardcoded YouTube geometry in the target design; warning-only disposition remains accepted. Runtime migration pending. (from ticket #6, `REQ-012` of #7)
  - **Update (#176, shipped):** the runtime migration of decision 2's "one home" has landed — the YouTube rectangles no longer live in `src/safe-area.ts`; they are read from the committed starter region file `examples/youtube-regions.json` through the single region ingestion point (src/composition-regions.ts) by a lazy, memoized reader (`protectedRegions()` in `src/safe-area.ts`) on first use, with no fallback to built-in numbers. The warning-only disposition and the structured-exclusion of the guideline overlay are unchanged; decision 1 and 3 stand.
