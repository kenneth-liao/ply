...
- Imported vectors are inert (#214, spec #207 US-006, DEC-007, TEST-005): the
  ...
  lists the first 20 and reports any beyond that bound) and the fix: embed
  the resource as a data URI. The scan is generic, not a per-element
  allowlist: every attribute value on every element is judged — any `url(…)`
  occurrence anywhere (paint servers on fill/stroke/filter/mask/clip-path),
  any `href`/`src`/`data`/`base`/`poster` value, each `srcset` candidate
  separately — plus `<style>` bodies, `<?xml-stylesheet?>`, and the DOCTYPE.
  Values are normalized in parser order before judging (XML entities decoded
  once, CDATA joined into the text it represents, CSS backslash escapes
  unescaped), so `&#104;ttps://…`, `@\69 mport`, `u\72 l(…)`, and CDATA-split
  keywords cannot hide a reference. DOCTYPE entity declarations (internal or
  external — a conformant parser expands internal entities, so
  entity-expansion-to-markup cannot be judged at text level) and unterminated
  constructs (comment, CDATA, script, style, PI, tag) refuse as malformed;
  nested `data:image/svg+xml` payloads are re-scanned once (bounded depth 1).
  Same-document `#id` fragments, embedded data URIs, the conventional DOCTYPE
  DTD identifier, plain text mentioning a URL, and a script (element, `src`,
  `on*` handler, `javascript:` href — never executed in the image path) do
  not block import. The scan is text-level and region-aware at the single
  ingestion point (`scanSvgExternalReferences` in src/svg-inertness.ts,
  called from `validateImageBytes` after the identity gate), so add, edit,
  and generated/matte byte ingestion all apply it; a refusal leaves nothing
  published. Inertness is proven, not assumed: rendering, measuring,
  reviewing, and replaying a script-only SVG issue zero network requests and
  no script side effect, observed through the render page's request log (now
  recorded and exposed from src/browser.ts).
...
- One-edit switch from a variable-font text Layer to a static face: `layer
  ...
  carried axes in the same edit, and a static face accepts `--width 100`
  (its implicit width) as well as its own weight — any other width is
...
- Added cross-Project Composition copy (#86, #77): `ply composition import <target> <source> --from-project <path>` copies a source Project Composition's reusable Layers into a destination Project as independent destination Layer identities with the retained bytes required for inspection/edit/render — one new identity per distinct source Layer (current revision only), duplicate source uses remapped through one identity map, source field values preserved, and no bundled-face or `assets/fonts/` resolution during the copy. Both Projects are locked in deterministic sorted-canonical-realpath order (reverse-direction imports serialize; a failed second acquisition releases the first), a source path resolving to the destination Project is refused with guidance to same-Project import, collisions fail closed with a byte-identical destination, publication stages content/revisions/identities and commits the destination Composition via one `atomicReplace` with caught-error cleanup of newly staged artifacts, and the destination remains inspectable/editable/renderable after the source Project and external inputs are removed.
...
- Added Composition reference removal and reordering without modifying Layers (#83, #77): `ply composition remove <comp> <use-name>` removes a use by local name without deleting or altering the underlying Layer, its historical revisions, or content blobs; `ply composition reorder <comp> --order <name1,name2,...>` permutes uses to adjust painting order, requiring an exact full-order permutation of existing use names and failing closed on invalid or malformed requests; both operations execute under `.ply.lock` and participate in the Project mutation protocol.
- Extended `docs/project-storage-contract.md` (§3, §5, §7) with the reference-list removal and reordering protocol, Layer preservation guarantees, and CLI commands (#83).
- Added in-place Layer editing through immutable revisions and explicit propagation (#82, #77): `ply layer edit <layer-id>` advances a Layer's current revision under its stable identity for local image and text content, placement, and opacity, preserving all historical revisions and content blobs; single-referrer and zero-referrer edits succeed without a flag, multi-referrer edits require explicit `--in-place` and name affected Compositions and their count on refusal, and referrer discovery scans under the Project lock with fail-closed handling for malformed or unreadable documents.
- Extended `docs/project-storage-contract.md` with the Layer editing protocol, fail-closed authoritative referrer discovery, and multi-referrer intent contract (#82).
